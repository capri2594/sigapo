...
$NXTSort_funcionarios = "funcionario.nombre";
...
function JJ_insertar(nomx,depx) { //v9.0
	...
	{ 
	  ...
	  alert(window.opener.self.document.getElementById('tr_nom').value);
...
?>
          <tr class="filter">
            <th><input type="text" name="dtable_DWAjaxTable1_nombre" value="<?php echo KT_escapeAttribute($dtable_DWAjaxTable1->getFilterValue('nombre')); ?>" size="30" maxlength="30" /></th>
...
?>
        ...
        <tbody class="data">
          ...
          <tr>
            <td colspan="5">que mal... esta vacio .</td>
          </tr>
          <?PHP } ?>
          <?php if ($totalRows_funcionarios > 0) { $num=0;?>
          <?php do { ?>
            <tr>
              <td id="nom<?php echo $num?>"><?php echo KT_FormatForList($row_funcionarios['nombre'], 30); ?></td>
              <td><?php echo KT_FormatForList($row_funcionarios['cargo'], 12); ?></td>
              <td id="dep<?php echo $num?>"><?php echo KT_FormatForList($row_funcionarios['cod'], 20); ?></td>
              <td>
                <input type="checkbox" name="checkbox" value="checkbox" onclick="JJ_insertar('nom<?php echo $num?>','dep<?php echo $num?>');"/>
              elegir</td>
            </tr>
            <?php $num=$num+1;} while ($row_funcionarios = mysql_fetch_assoc($funcionarios)); ?>
          <?php }?>
        </tbody>
      </table>
    <script type="text/javascript">
