...
if (isset($_GET['cuenta'])) {
  ...
  echo "Se ha cambiado la contraseña correctamente, gracias."; 
}

}// fin verificacion de la contraseña nueva
else{
// ni hay la constrseña nueva no coincide.
  echo "Error 405: La constraeña actual no coincide o es vacia. Revisar e intentar nuevamente."; 
}

}else{
// si el usuario no existe en el sistema...
  echo "Error 404: EL usuario o contrasenia es incorrecto. Verifique e intente nuevamente"; 
  //exit();
...
<form id="form1" name="form1" method="GET" action="<?php echo $editFormAction; ?>">
  ...
  <input type="text" name="old_pswd" id="old_pswd" />
  </label>
  <input type="text" name="new_pswd" id="new_pswd" />
  <label>
  <input type="text" name="repeat_new_pswd" id="repeat_new_pswd" />
  </label>
