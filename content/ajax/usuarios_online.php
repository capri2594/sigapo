...
mysql_query("delete from session where tiempo < $limite") or die(mysql_error());
// tomamos todos los usuarios en linea
//$resp = mysql_query("select * from session where ip='$ip'") ;
//CONTROL POR USUARIO
$resp = mysql_query("select * from session where cuenta='$usuario'") or die(mysql_error());
// Si son los mismo actualizamos la tabla gente_online
if(mysql_num_rows($resp) != 0) {
//mysql_query("update session set tiempo='$date' where ip='$ip'") ;
mysql_query("update session set tiempo='$date' where cuenta='$usuario'") or die(mysql_error());
}
// de lo contrario insertamos los nuevos
else {

mysql_query("insert into session (cuenta,session_id,tiempo,ip) values ('$usuario','$session_id','$date','$ip')") or die(mysql_error());
}
// Seleccionamos toda la tabla
$query = "SELECT * FROM session WHERE cuenta!=''";
