...
$colname_buscarHR = "-1";
if (isset($_GET['cod'])) {
  $colname_buscarHR = $_GET['cod'];
}
$colname_gestion = "-1";
if (isset($_GET['gestion'])) {
  $colname_gestion = $_GET['gestion'];
}
mysql_select_db($database_snet, $snet);
$query_buscarHR = sprintf("SELECT cod FROM hojaruta WHERE cod LIKE %s AND gestion=%s ORDER BY cod ASC", GetSQLValueString($colname_buscarHR."%", "text"),GetSQLValueString($colname_gestion, "text"));
