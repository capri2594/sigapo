...
$dtable_DWAjaxTable1 = new dtable($conn_snet, 'funRemitentes', 'dtable_DWAjaxTable1', 'funcionario');
...
$colname_funRemitentes = "-1";
if (isset($_GET['coddep'])) {
  $colname_funRemitentes = $_GET['coddep'];
}
...
$query_funRemitentes = sprintf("SELECT nombre, cargo, dependencia.nombredep, funcionario.dependencia_cod  FROM funcionario, dependencia WHERE funcionario.dependencia_cod=dependencia.cod and funcionario.dependencia_cod = '%s'  AND  %s ORDER BY %s ", $colname_funRemitentes,$NXTFilter_funRemitentes,$NXTSort_funRemitentes);
...
function JJ_insertar(nomX,depX) { //v9.0
   ...
   //alert('innerHTML:'+document.getElementById(nom).innerHTML);
	...
	{ 
	  ...
	  alert(objdep.innerHTML);
	  alert(window.opener.self.document.getElementById('fun_remite').innerHTML);*/
	  window.opener.self.document.getElementById('fun_remite').value=objnom.innerHTML;
	  window.opener.self.document.getElementById('dep_remite').value=depX;
...
<form class="dtable" id="<?php echo $dtable_DWAjaxTable1->listName; ?>" action="<?php echo KT_escapeAttribute($dtable_DWAjaxTable1->getFilterUri()); ?>" method="get">
  <div> <?php echo $dtable_DWAjaxTable1->beginList(); ?>
     <span class="Estilo1">CODIGO UNIDAD/DEPENDENCIA:</span><div id="div_coddep" name="div_coddep"></div> 
     <div align="left" style="text-transform:uppercase; font-weight:bold;"><?php echo $row_funRemitentes['nombredep'];?><br />
     </div>
<table border="0" cellpadding="0" cellspacing="0">
        ...
        <thead>
		  ...
		  <tr class="filter">
            <th><input type="text" name="dtable_DWAjaxTable1_nombre" value="<?php echo KT_escapeAttribute($dtable_DWAjaxTable1->getFilterValue('nombre')); ?>" size="30" maxlength="50" /></th>
...
?>
        ...
        <tbody class="data">
          ...
          <tr>
            <td colspan="4">No se encontraron datos que mostrar. Intentelo nuevamente </td>
          </tr>
          <?PHP } ?>
          <?php if ($totalRows_funRemitentes > 0) { $num=0;?>
          <?php do { ?>
            <tr>
              <td id="nom<?php echo $num?>"><?php echo KT_FormatForList($row_funRemitentes['nombre'], 50); ?></td>
              <td><?php echo KT_FormatForList($row_funRemitentes['cargo'], 20); ?></td>
              <td><a href="javascript:void(0);" onclick="JJ_insertar('nom<?php echo $num?>','<?php echo KT_FormatForList($row_funRemitentes['nombredep'], 30); ?>');">SELECCIONAR</a></td>
            </tr>
            <?php $num=$num+1;} while ($row_funRemitentes = mysql_fetch_assoc($funRemitentes)); ?>
          <?php }?>
        </tbody>
      </table>
    <script type="text/javascript">
