...
if (isset($_GET['inicio'])) {
  $hoy_mis_recibidos = $_GET['inicio'];
}

$fin_mis_recibidos=$_GET['fin'];

//$hoy_mis_recibidos = date("Y-m-d");

mysql_select_db($database_snet, $snet);
$query_mis_recibidos = sprintf("SELECT * FROM entradas, einterna WHERE entradas.id=einterna.entradas_id AND entradas.cod_deprecibido=%s  AND entradas.fecha_recibido>%s AND entradas.fecha_recibido<=%s", GetSQLValueString($colname_mis_recibidos, "text"),GetSQLValueString($hoy_mis_recibidos, "date"),GetSQLValueString($fin_mis_recibidos, "date"));
...
if (isset($_GET['inicio'])) {
  $hoy_mostrar_externos = $_GET['inicio'];
}
$fin_mostrar_externos = $_GET['fin'];
mysql_select_db($database_snet, $snet);
$query_mostrar_externos = sprintf("SELECT * FROM entradas, eexterna WHERE entradas.id=eexterna.entradas_id AND entradas.cod_deprecibido=%s  AND (entradas.fecha_recibido>%s AND entradas.fecha_recibido<=%s)", GetSQLValueString($colname_mostrar_externos, "text"),GetSQLValueString($hoy_mostrar_externos, "date"),GetSQLValueString($fin_mostrar_externos, "date"));
...
if (isset($_GET['inicio'])) {
  $hoy_mis_hojasrutas = $_GET['inicio'];
}
$fin_mis_hojasrutas = $_GET['fin'];
mysql_select_db($database_snet, $snet);
$query_mis_hojasrutas = sprintf("SELECT * FROM entradas, derivacion, hojaruta WHERE entradas.id=derivacion.entradas_id AND entradas.cod_deprecibido=%s  AND entradas.fecha_recibido>%s AND entradas.fecha_recibido<=%s AND derivacion.hojaruta_cod=hojaruta.cod", GetSQLValueString($colname_mis_hojasrutas, "text"),GetSQLValueString($hoy_mis_hojasrutas, "date"),GetSQLValueString($fin_mis_hojasrutas, "date"));
        ...
        <?php if ($totalRows_mis_hojasrutas > 0) { // Show if recordset not empty ?>
          <?php do { ?>
            <row>
              ...
              <anexos><?php echo $row_mis_hojasrutas['nanexos']; ?></anexos>
