...
function JJ_insertar(nomX,depX) { //v9.0
   ...
   //alert('innerHTML:'+document.getElementById(nom).innerHTML);
	...
	{ 
	  ...
	  alert(window.opener.self.document.getElementById('fun_remite').innerHTML);*/
	  window.opener.self.document.getElementById('fun_remite').innerHTML=objnom.innerHTML;
	  window.opener.self.document.getElementById('dep_remite').innerHTML=depX;
...
.over{
 background-color:#B8BDE4;/*#FEEFB1;*/
 cursor:hand;
 }
.normal{
...
<form class="dtable" id="<?php echo $dtable_DWAjaxTable1->listName; ?>" action="<?php echo KT_escapeAttribute($dtable_DWAjaxTable1->getFilterUri()); ?>" method="get">
  <div> <?php echo $dtable_DWAjaxTable1->beginList(); ?>
     <span class="Estilo1">CODIGO UNIDAD/DEPENDENCIA:</span>&nbsp; <div id="div_coddep" name="div_coddep"> <?php echo $_GET[coddep];?>
      </div> <br />
...
?>
        ...
        <tbody class="data">
          ...
          <?php do { ?>
            <tr id="filas" onmousemove="this.className='over';" onmouseout="this.className='normal';" onclick="JJ_insertar('nom<?php echo $num?>','<?php echo KT_FormatForList($row_funRemitentes['nombredep'], 20);?>');">
              <td id="nom<?php echo $num?>"><?php echo KT_FormatForList($row_funRemitentes['nombre'], 30); ?></td>
              <td><?php echo KT_FormatForList($row_funRemitentes['cargo'], 20); ?></td>
              <td><a href="javascript:void(0);" onclick="JJ_insertar('nom<?php echo $num?>','<?php echo KT_FormatForList($row_funRemitentes['nombredep'], 20);?>');">SELECCIONAR</a></td>
            </tr>
