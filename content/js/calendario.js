	...
	// traducido al español por: Jose Luis Aranibar Araviri
	...
	var calendars = Array();
	var fc_months = Array('Enero', 'Febero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre');
	var fc_openCal;
	...
	function fc_show() {
		...
		if (typeof(this.div) == 'undefined') {
			...
			//el codigo fue modificado porque en FIREFOX no aparecia las cordenadas left,top del objeto, se calculaba correctamente pero lo que no hacia era asignar por un pequeño detalle aumentar 'px' para que sea valor px. Ej.  100px, y recien se asigno y funciono correctamente.
			// fecha de modificacion: 11 de MARZO de 2008, martes
			// modificado por: Jose Luis Aranibar Araviri.
			
			
			//alert(fc_absoluteOffsetLeft(this.element));
			//alert(fc_absoluteOffsetTop(this.element) +' '+this.element.offsetHeight +' 1');
			this.div.style.left = fc_absoluteOffsetLeft(this.element)+'px';/*'110px';*/
			var suma=fc_absoluteOffsetTop(this.element)+this.element.offsetHeight+1;
			//alert(suma);
			//alert('left:'+this.div.style.left+' top:'+suma);
			this.div.style.top = fc_absoluteOffsetTop(this.element)+this.element.offsetHeight+1+'px';/*'180px';*/
			//alert('left:'+this.div.style.left+' top:'+this.div.style.top);
