...
mysql_select_db($database_snet, $snet);
$query_salidas_derivadas = sprintf("SELECT * FROM salidas, derivacion WHERE derivacion.salidas_id=salidas.id  AND salidas.fecha_envio>=%s AND salidas.fecha_envio<=%s AND derivacion.cod_depderivador=%s", GetSQLValueString($hoyA_salidas_derivadas, "date"),GetSQLValueString($hoyB_salidas_derivadas, "date"),GetSQLValueString($coddep_salidas_derivadas, "text"));
...
$totalRows_salidas_derivadas = mysql_num_rows($salidas_derivadas);
  ...
  $hoyA_num_en_arch = $hoyA;


  $hoyB_num_en_arch = $hoyB;

mysql_select_db($database_snet, $snet);
$query_num_en_arch = sprintf("SELECT count(entradas.id) FROM derivacion, entradas WHERE derivacion.entradas_id=entradas.id AND derivacion.proveido='Archivo' AND derivacion.cod_depderivador=%s  AND (entradas.fecha_recibido>=%s AND entradas.fecha_recibido<=%s)",GetSQLValueString($coddep_salidas_derivadas, "text"), GetSQLValueString($hoyA_num_en_arch, "date"),GetSQLValueString($hoyB_num_en_arch, "date"));
...
$datay=array($totalRows_entradas,$totalRows_salidas_derivadas,($totalRows_entradas-$totalRows_salidas_derivadas));
...
$bplot->SetFillColor(array('#FFFFCC','#CCFFCC', '#FFCC99'));
$bplot->value->Show();
$graph->Add($bplot);
...
$labels=array("Entradas","Salidas","Pendientes");
