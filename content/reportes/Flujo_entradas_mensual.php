...
mysql_select_db($database_snet, $snet);
$query_n_entradas = sprintf(" SELECT count( * ) as total FROM `entradas` WHERE YEAR(`fecha_recibido`)= %s AND MONTH(`fecha_recibido`)= %s AND `cod_deprecibido`=%s", GetSQLValueString($ges, "int"),GetSQLValueString($mes, "int"),GetSQLValueString($cod[$i], "text"));
...
// Setup title
$graph->title->Set("GOBIERNO AUTONOMO DPTAL. DE ORURO \n Flujo de Entradas Mensual");
$graph->title->SetFont(FF_VERDANA,FS_BOLD,12);
//$graph->title->Set("");
$graph->subtitle->Set("Reporte: ".$mes."-".$ges."\n\n");
...
$graph->yaxis->scale->SetGrace(20);//era 20
...
<ul>
  <?php do { ?>
    <li><?php echo $row_dep['cod']; ?>        </li>
    <?php } while ($row_dep = mysql_fetch_assoc($dep)); ?></ul>
</body>
</html>
<?php
mysql_free_result($dep);

mysql_free_result($n_entradas);
?>
