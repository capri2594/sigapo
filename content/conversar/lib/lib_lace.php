        ...
        if (LACE_SHOW_JOIN_PART)
        {
                ...
                if($name && false === $A->keyExists($name))
                {
                        ...
                        (
                                ...
                                'time'   => time(),
                                'name'   => 'Lace',
                                'text'   => '<strong>'.$name.'</strong>se une a la conversación',
                        );
                        addMessage($message);
                }
        }
    ...
    $hourString = '';
        ...
        // $fileContents array is empty
        {
                ...
                $welcome      = ($file == LACE_FILE)
                        ? 'Bienvenido a '.LACE_SITE_NAME.'. Nadie ha dicho nada todavía.'
                        : 'El registro actual está vacía.';
                $finalOutput .= (file_exists($file))
                        ? $welcome
                        : 'Lo sentimos, el archivo que ha pedido no existe.';
        }
...
function addNameChange($from, $to)
{
        ...
        if (LACE_SHOW_NAME_CHANGE)
        {
                ...
                (
                        ...
                        'time'   => time(),
                        'name'   => 'Lace',
                        'text'   => '<strong>'.$from.'</strong> es ahora <strong>'.$to.'</strong>',
                );
